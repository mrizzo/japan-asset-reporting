...
A browser-based interface is included. Start it with:

```bash
make serve
```

Then open `http://localhost:8000`. Upload `assets.csv` and `config.csv` and click **Generate Forms** — a zip of all four PDFs is returned.

**My Number is never written to the output.** Write it in by hand on each form before filing.

To password-protect the web app, set the `ACCESS_CODE` environment variable before starting:

```bash
ACCESS_CODE=yourpasscode make serve
```
